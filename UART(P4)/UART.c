...
{// This function uses recursion to convert decimal number
 //   of unspecified length as an ASCII string
	...
	unsigned int num=0, rem=0, i=0;
	char arr[7] = {'"', ' ', ' ', ' ', ' ', ' ', '"'};
	// the format of output string : "nnnn "
	// The place of 'n' will be replaced to a number or '*' as following code.  
	
	num = n;
	for(i=4;i>=1;i--)
	{
		if(n >10000) 				// incase an input is over 10000
			arr[i] = '*';			// replace arr[i] to '*'
		else if(n==0 && i==4)
			arr[i] = '0';
...
void UART_OutDistance(unsigned long n)
{
	unsigned int num=0, rem=0, i=0;
	char arr[11] = {'"', ' ', '.', ' ', ' ', ' ', ' ', ' ', 'c', 'm', '"'};
	// the format of output string : "n.nnn cm"
	// The place of 'n' will be replaced to a number or '*' as following code.  
	num = n;
	for(i=5;i>=1;i--)
	{
		if(i==2);				// arr[2] is fixed to '.'
		else if(n >10000)	// in case an input is over 10000
		{
			arr[i] = '*';	// replace arr[i] to '*'
		}
		else if(num>0) // replace arr[i] to a number
		...
		else					//replace arr[i] to significant zeros.
			arr[i] = '0';
	}
	UART_OutString(arr);
}
