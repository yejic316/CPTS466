...
//  UART0_FBRD_R = 8;                     // FBRD = int(0.1267 * 64 + 0.5) = 8
  ...
  UART1_FBRD_R = 53;                     // FBRD = int(0.8333 * 64) = 53
...
//-----------------------UART_OutDouble----------------------
// Output a 32-bit number in double (nn.nn) format
void UART_OutDouble(long double n)
{
	int i=0;
	unsigned int num=0, rem=0;
	char arr[6] = {' ', ' ', '.', ' ', ' ', ' '};
	
	num=n*100; // nn.nn -> nnnn
	for(i=4;i>=0;i--)
	{
		if(i==2)
		{
		...
		else if(num>0)			// replace arr[i] to a number
		{
			rem=num%10;
			num=num/10;
			arr[i] =rem+'0';
